...
class CupyMemory:
    ...
    def __init__(self, capacity: int,
        ...
        self.s_len = len(state_shapes)
        self.s = [None] * self.s_len
        self.s_ = [None] * self.s_len

        for i in range(self.s_len):
            s_shapes = [capacity] + list(state_shapes[i])
            self.s[i] = cp.full(s_shapes, 0.0)
            self.s_[i] = cp.full(s_shapes, 0.0)

        action_shape = [capacity] + list(action_shape)
        self.a = cp.full(action_shape, 0.0)

        self.r = cp.full((capacity, 1), 0.0)
        self.d = cp.full((capacity, 1), False)
    ...
    def append(self, s: Union[list, tuple, int, float],
        ...
        if isinstance(s, (int, float)):  # state is an int
            ...
            s_ = [s_]
        for i in range(self.s_len):
            self.s[i][self.ptr] = cp.array(s[i])
            self.s_[i][self.ptr] = cp.array(s_[i])

        self.a[self.ptr] = cp.array(a)
        self.r[self.ptr] = r
        self.d[self.ptr] = d
